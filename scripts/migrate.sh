...
# Restores 622 Gatekeeper-disabled casks (fails_gatekeeper_check) with quarantine removal.
# Usage:
...
do
  case "${arg}" in
    ...
    --list) LIST_ONLY=1 ;;
    -h | --help)
      echo "Usage: $0 [--yes] [--interactive] [--dry-run] [--list]"
      exit 0
      ;;
    *)
...
# Ensure tap
if ! brew tap | grep -q "thedavidweng/unsigned-tap"
then
  echo "==> Tapping ${TAP} ..."
  brew tap "${TAP}"
else
...
do
  if echo " ${DISABLED_TOKENS} " | grep -q " ${cask} "
  then
...
then
  ...
  echo "You have ${#INSTALLED} casks, none match the 622 disabled (fails_gatekeeper_check)."
